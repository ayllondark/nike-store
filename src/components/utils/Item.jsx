...
const dispatch = useDispatch();
  ...
  return (
    <>
        <div className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists ? 'justify-items-start' : 'justify-items-center'} rounded-xl py-4 px-5 transition-all duration-700 easy-in-out w-full hover:scale-105`}>
            <div className={`grid items-center ${ifExists ? 'justify-items-start' : 'justify-items-center'}`}>
                ...
                <div className='flex items-center justify-between w-28 my-2'>
                    ...
                    <div className='flex items-center gap-1'><StarIcon className='icon-style w-5 md:w-4 md:h-4 text-slate-100' /><h1 className='md:text-sm font-normal text-slate-100'>{rating}</h1></div>
                </div>
                ...
                <div className='flex items-center gap-3'>
                    ...
                    <button type='button' className='bg-white/90 blur-effect-theme button-theme px-2 py-2 shadow shadow-sky-200 text-sm text-black' onClick={()=> {onAddToCart(); onCartToogle();}}>{btn}</button>
                </div>
